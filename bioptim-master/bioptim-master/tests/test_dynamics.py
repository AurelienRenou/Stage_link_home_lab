...
def test_torque_driven_implicit_soft_contacts(with_contact, cx, implicit_contact):
    ...
    if with_contact:
        ...
        np.testing.assert_almost_equal(
            x_out[:, 0], [0.6118529, 0.785176, 0.6075449, 0.8083973, -0.3024137, -10.3850379, 1.6044517, 35.8023864],
        )
...
@pytest.mark.parametrize(
    "dynamics", [DynamicsFcn.TORQUE_ACTIVATIONS_DRIVEN, DynamicsFcn.MUSCLE_DRIVEN],
)
def test_implicit_soft_contacts_errors(dynamics):
    ...
    nlp.control_type = ControlType.CONSTANT
    NonLinearProgram.add(
        ocp, "dynamics_type", Dynamics(dynamics, implicit_soft_contacts=True), False,
    )
...
def test_implicit_dynamics_errors(dynamics):
    ...
    nlp.control_type = ControlType.CONSTANT
    NonLinearProgram.add(
        ocp, "dynamics_type", Dynamics(dynamics, implicit_dynamics=True), False,
    )
...
def test_torque_activation_driven(with_contact, with_external_force, cx):
    ...
    if with_contact:
        ...
        if with_external_force:
            np.testing.assert_almost_equal(
                x_out[:, 0], [0.8631, 0.32518, 0.11959, 0.4938, 19.01887, 18.51503, -53.08574, 58.48719], decimal=5,
            )
    ...
    if with_contact:  # Warning this test is a bit bogus, there since the model does not have contacts
        if with_torque:
            ...
            if with_excitations:
                ...
                if with_external_force:
                    np.testing.assert_almost_equal(
                        x_out[:, 0], [0.6158501, 0.50313626, 0.64241928, 1.31194581, -50.56193318, 82.71912199],
                    )
                else:
                    np.testing.assert_almost_equal(
                        x_out[:, 0],
