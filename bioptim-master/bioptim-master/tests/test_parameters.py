...
def test_param_scaling():
    ...
    with pytest.raises(
        ValueError, match="Parameter scaling must be a numpy array",
    ):
    ...
    with pytest.raises(
        ValueError, match="Parameter scaling must contain only positive values",
    ):
        param.add("gravity_z", my_parameter_function, initial_gravity, bounds, size=1, scaling=np.array([-1]))

    with pytest.raises(
        ValueError, match="Parameter scaling must be a 1- or 2- dimensional numpy array",
    ):
        param.add("gravity_z", my_parameter_function, initial_gravity, bounds, size=1, scaling=np.array([[[1]]]))
