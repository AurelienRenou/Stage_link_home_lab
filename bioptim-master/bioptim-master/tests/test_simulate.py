...
def test_interpolate():
    ...
    ocp = ocp_module.prepare_ocp(
        biorbd_model_path=bioptim_folder + "/models/pendulum.bioMod", final_time=2, n_shooting=n_shooting,
    )
...
def test_integrate_single_shoot_use_scipy(keep_intermediate_points, ode_solver):
    ...
    if ode_solver == OdeSolver.RK4:
        np.testing.assert_almost_equal(
            sol_integrated.states["q"][:, [0, -1]], np.array([[0.0, -0.40229917], [0.0, 2.66577734]]), decimal=decimal,
        )
        np.testing.assert_almost_equal(
        ...
        np.testing.assert_almost_equal(
            sol_integrated.states["q"][:, [0, -1]], np.array([[0.0, -0.93010486], [0.0, 1.25096783]]), decimal=decimal,
        )
        np.testing.assert_almost_equal(
            sol_integrated.states["qdot"][:, [0, -1]],
