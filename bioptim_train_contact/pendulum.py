...
def prepare_ocp(
    ...
    x_bounds[:, [0, -1]] = 0
    x_bounds[1, -1] = 3.14  # initial position is 0 rad, we want that the pendulum is in a vertical position
    x_bounds.min[2:, 1] = -90  # we have widened the qdot bounds
    x_bounds.max[2:, 1] = 90
    ...
    tau_min, tau_max, tau_init = -200, 200, 0  # we have widened the bounds in order to make the problem converge
    u_bounds = Bounds([tau_min] * n_tau, [tau_max] * n_tau)
    u_bounds[0, :] = 0  # Prevent the model from actively transpose
